...
class SinkRecordToArrowConverterTimestampTest {
    ...
    class AutomaticTimestampDetectionTests {
        ...
        void shouldDetectTimestampsInJSONData() {
            ...
            String jsonValue = "{\"id\": 123, \"created_at\": \"2025-09-25T18:05:12Z\", \"name\": \"test\"}";

            SinkRecord record = new SinkRecord(
                "test-topic", 0, null, null, null, jsonValue, 0L
            );
            ...
            try (VectorSchemaRoot result = converter.convertRecords(List.of(record))) {
                ...
                assertInstanceOf(ArrowType.Timestamp.class, createdAtField.getType(), "created_at field should be detected as timestamp");
        ...
        void shouldDetectTimestampsInMapData() {
            ...
            SinkRecord record = new SinkRecord(
                "test-topic", 0, null, null, null, mapValue, 0L
            );
            ...
            try (VectorSchemaRoot result = converter.convertRecords(List.of(record))) {
                ...
                TimeStampMilliVector updatedAtVector = (TimeStampMilliVector) result.getVector("updated_at");

                long expectedTimestamp = TimestampUtils.parseTimestampToEpochMillis("2025-09-25T18:05:12");
                long expectedUpdatedAt = TimestampUtils.parseTimestampToEpochMillis("2025-09-25T20:30:45.123Z");
        ...
        void shouldHandleMixedTimestampFormats() {
            ...
            try (VectorSchemaRoot result = converter.convertRecords(List.of(record))) {
                ...
                for (int i = 1; i <= 4; i++) {
                    ...
                    assertNotNull(field);
                    assertInstanceOf(ArrowType.Timestamp.class, field.getType(), "ts" + i + " should be detected as timestamp");
                }

                // Last one should be string
                Field notTimestampField = schema.findField("not_timestamp");
                assertNotNull(notTimestampField);
                assertInstanceOf(ArrowType.Utf8.class, notTimestampField.getType(), "not_timestamp should remain as string");
    ...
    class StructuredDataWithTimestampsTests {
        ...
        void shouldHandleExplicitTimestampSchema() {
            ...
            SinkRecord record = new SinkRecord(
                "test-topic", 0, null, null, schema, struct, 0L
            );
        ...
        void shouldHandleTimestampStringsInStructuredData() {
            ...
            Struct struct = new Struct(schema);
            struct.put("id", 123L);
            struct.put("created_at", new java.util.Date(TimestampUtils.parseTimestampToEpochMillis("2025-09-25T18:05:12Z")));
            struct.put("name", "test record");
        ...
        private static SinkRecord getSinkRecord() {
            ...
            return new SinkRecord(
                "test-topic", 0, null, null, null, rootObject, 0L
            );
        }
    }
    ...
    class ErrorHandlingTests {
        ...
        void shouldHandleEmptyRecords() {
            ...
            SinkRecord record = new SinkRecord(
                "test-topic", 0, null, null, null, emptyMap, 0L
            );
    ...
    class MultipleRecordsTests {
        ...
        void shouldHandleMultipleRecordsWithConsistentTimestamps() {
            ...
            try (VectorSchemaRoot result = converter.convertRecords(records)) {
                ...
                for (int i = 0; i < 3; i++) {
                    ...
                    long expected = TimestampUtils.parseTimestampToEpochMillis("2025-09-25T" + String.format("%02d", 8 + i) + ":05:12Z");
        ...
        void shouldHandleSchemaEvolutionWithTimestamps() {
            ...
            try (VectorSchemaRoot result = converter.convertRecords(records)) {
                ...
                TimeStampMilliVector createdAtVector = (TimeStampMilliVector) result.getVector("created_at");
                TimeStampMilliVector updatedAtVector = (TimeStampMilliVector) result.getVector("updated_at");
