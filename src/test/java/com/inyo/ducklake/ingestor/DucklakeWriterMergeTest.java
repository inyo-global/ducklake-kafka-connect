...
class DucklakeWriterMergeTest {
  ...
  private VectorSchemaRoot root(int[] ids, String[] names) {
    ...
    Schema schema = new Schema(List.of(fId, fName));
  ...
  private String nameForId(String table, int id) throws Exception {
    try (PreparedStatement ps =
        conn.prepareStatement("SELECT name FROM " + table + " WHERE id=?")) {
      ps.setInt(1, id);
      try (ResultSet rs = ps.executeQuery()) {
  ...
  void testMergeUpsert() throws Exception {
    DucklakeWriterConfig cfg =
        new DucklakeWriterConfig("t_upsert", true, new String[] {"id"}, new String[0]);
