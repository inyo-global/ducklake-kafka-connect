...
public final class DucklakeWriter implements AutoCloseable {
    ...
    private void upsertWithMergeInto(VectorSchemaRoot root, List<Field> fields, String[] pkCols)
        ...
        try (var arrayStream = ArrowArrayStream.allocateNew(allocator)) {
            ...
            var usingClause = java.util.Arrays.stream(pkCols)
                    .map(SqlIdentifierUtil::quote)
                    .collect(Collectors.joining(", "));

            var pkSet = java.util.Arrays.stream(pkCols)
                    .map(c -> c.toLowerCase(java.util.Locale.ROOT))
                    .collect(java.util.stream.Collectors.toSet());
            var nonKey = fields.stream()
                    .filter(f -> !pkSet.contains(f.getName().toLowerCase(java.util.Locale.ROOT)))
                    .toList();
            var updateSetClause = nonKey.isEmpty() ? null : nonKey.stream()
                    .map(f -> SqlIdentifierUtil.quote(f.getName()) + " = " + tempTable + "." + SqlIdentifierUtil.quote(f.getName()))
                    .collect(Collectors.joining(", "));
            var insertCols = fields.stream().map(f -> tempTable + "." + SqlIdentifierUtil.quote(f.getName())).collect(Collectors.joining(", "));
            var sql = new StringBuilder();
            sql.append("MERGE INTO ").append(tableQuoted).append(" USING ").append(tempTable).append(" USING (").append(usingClause).append(") ");
