...
/**
 * Configuration record for DucklakeWriter containing all necessary settings
 * for writing data to DuckDB tables.
 *
...
public record DucklakeWriterConfig(
    ...
    public String toString() {
        return String.format("DucklakeWriterConfig{destinationTable='%s', autoCreateTable=%s, tableIdColumns=%s, partitionByColumns=%s}",
                destinationTable, autoCreateTable, Arrays.toString(tableIdColumns), Arrays.toString(partitionByColumns));
    }
    ...
    @Override
    public int hashCode() {
        int result = destinationTable.hashCode();
        result = 31 * result + Boolean.hashCode(autoCreateTable);
