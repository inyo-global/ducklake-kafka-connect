...
public class ArrowSchemaMerge {
    ...
    /**
     * Unifies multiple Arrow schemas into a single schema.
     * Fields with the same name are merged, and type conflicts are resolved.
     *
    ...
    private static Field mergeFields(String fieldName, List<Field> fields) {
        ...
        Set<ArrowType> types = fields.stream()
    ...
    /**
     * Unifies different Arrow types into a compatible type.
     * Follows type promotion rules similar to PyArrow.
     *
    ...
    private static ArrowType promoteNumericTypes(Set<ArrowType> types) {
        ...
        boolean hasInt64 = types.stream().anyMatch(t ->
            t instanceof ArrowType.Int && ((ArrowType.Int) t).getBitWidth() == 64);
        boolean hasInt32 = types.stream().anyMatch(t ->
            t instanceof ArrowType.Int && ((ArrowType.Int) t).getBitWidth() == 32);

        if (hasFloatingPoint) {
            // Check if we have double precision
            boolean hasDouble = types.stream().anyMatch(t ->
                t instanceof ArrowType.FloatingPoint &&
                ((ArrowType.FloatingPoint) t).getPrecision() == FloatingPointPrecision.DOUBLE);
    ...
    /**
     * Creates a new schema by adding a field to an existing schema.
     * If a field with the same name exists, it will be merged.
     *
    ...
    public static Schema addField(Schema schema, Field newField) {
        ...
        Optional<Field> existingField = existingFields.stream()
                .filter(f -> f.getName().equals(newField.getName()))
                .findFirst();

        if (existingField.isPresent()) {
            // Merge with existing field
            existingFields.remove(existingField.get());
            Field mergedField = mergeFields(newField.getName(),
                    Arrays.asList(existingField.get(), newField));
    ...
    public static Schema removeField(Schema schema, String fieldName) {
        List<Field> filteredFields = schema.getFields().stream()
