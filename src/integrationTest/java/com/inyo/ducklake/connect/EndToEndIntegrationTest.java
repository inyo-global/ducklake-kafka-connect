...
class EndToEndIntegrationTest {
    ...
    public static final KafkaContainer kafkaContainer =
            new KafkaContainer("apache/kafka-native:4.0.0").withNetwork(network).withNetworkAliases("kafka");
    ...
    @Test
    void connector() {
        // Build MinIO client using host and mapped port so JVM can reach the container; use known credentials
        var minioHost = minio.getHost();
        var minioPort = minio.getMappedPort(9000);
        var endpoint = String.format("http://%s:%d", minioHost, minioPort);
        try (var minioClient = MinioClient.builder().endpoint(endpoint).credentials("minio", "minio123").build()) {
            boolean exists = minioClient.bucketExists(io.minio.BucketExistsArgs.builder().bucket("test-bucket").build());
            if (!exists) {
        ...
        // Build connector configuration map and start it via Kafka Connect REST using DucklakeSinkConfig
        ...
        Awaitility.await()
                .untilAsserted(
                        () -> {
                            // Build a config map matching the connector settings so DucklakeConnectionFactory can attach
    ...
    private static @NotNull DucklakeSinkConfig getDucklakeSinkConfig() {
        ...
        connectorProps.put("ducklake.catalog_uri", "postgres:dbname=ducklake_catalog host=postgres user=duck password=duck");
